...
import com.spring.web.ColorController;
...
privileged aspect ColorController_Roo_Controller {
    ...
    public String ColorController.create(@Valid Color color, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        uiModel.asMap().clear();
        color.persist();
        return "redirect:/colors/" + encodeUrlPathSegment(color.getId().toString(), httpServletRequest);
    }
    ...
    public String ColorController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("colors", Color.findColorEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) Color.countColors() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("colors", Color.findAllColors(sortFieldName, sortOrder));
        }
        return "colors/list";
    }
